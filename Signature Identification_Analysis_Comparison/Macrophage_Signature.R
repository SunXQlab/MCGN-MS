...
# fit = rbsurv(time=time, status=status, x=Gene_marker, method="efron", max.n.genes=20, n.iter=10, n.fold=3)
# fit$model
# Gene_sig_id = as.numeric(fit$gene.list)
# Marker = Gene_marker[Gene_sig_id,]
# Gene_signature = rownames(MNB[Gene_sig_id])
...
# "PXDN"   "CDH6"   "SCN3A"  "ANPEP"  "SEMA6B" "CCDC37" "DPP4"   "PRRG1"  "GPNMB"  "FANCA"  "TMEM26" "NETO2"
# 0.0111123294  0.0008619239 -0.8772969022  0.0016958263 -0.0423078649  0.0196739563  0.0013511639  0.8284922208  0.0026937362  1.1843625406 0.5722500646  0.1010323338
...
for (i in 1:12)
{
fit <- coxph(Surv(time, status) ~ (Gene_marker[which(A!=0)[12],]), method="breslow")
# fit
...
# 
# 
# LASSO_fit<-glmnet(t(Gene_marker),surv,family="cox")
# 
# dev.new()
# plot(LASSO_fit,label=F, xvar='lambda' ,pch=25,ylim = c(-10,5))  #, xvar='lambda'
